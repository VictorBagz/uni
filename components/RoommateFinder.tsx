...
// --- Helper Components ---
const SectionTitle = ({ children }) => <h3 className="text-xl font-bold text-unistay-navy mb-4 border-b pb-2">{children}</h3>;
const Input = (props) => <input {...props} className="w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-unistay-yellow focus:border-unistay-yellow disabled:bg-gray-200" />;
const Select = ({ children, ...props }) => <select {...props} className="w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-unistay-yellow focus:border-unistay-yellow disabled:bg-gray-200">{children}</select>;
const Textarea = (props) => <textarea {...props} rows={4} className="w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-unistay-yellow focus:border-unistay-yellow disabled:bg-gray-200" />;
const Label = ({ children, ...props }) => <label {...props} className="block text-sm font-medium text-gray-700 mb-1">{children}</label>;


// --- Profile Creation/Editing Form ---
const ProfileForm = ({ profile, user, onProfileUpdate, onCancel }) => {
    ...
    return (
        <div className="bg-white rounded-2xl shadow-xl p-6 md:p-8 animate-fade-in">
            <h2 className="text-2xl font-bold text-unistay-navy mb-1">{profile ? 'Update Your Profile' : 'Create Your Roommate Profile'}</h2>
            <p className="text-gray-600 mb-6">A detailed profile helps us find you the best matches!</p>
            <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <div className="flex justify-end gap-4 pt-4 border-t">
                    {profile && <button type="button" onClick={onCancel} className="bg-gray-200 text-gray-800 font-bold py-2 px-6 rounded-full hover:bg-gray-300 transition-colors">Cancel</button>}
...
const RoommateFinder = ({ currentUser, currentUserProfile, onProfileUpdate, profiles, universities, onNavigateHome }: RoommateFinderProps) => {
    ...
    const showProfileForm = !currentUserProfile || isEditing;
